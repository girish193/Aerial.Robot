...
int main(int argc, char **argv)
{
   ...
   // pose_vel.type_mask =  pose_vel.IGNORE_AFX | pose_vel.IGNORE_AFY | pose_vel.IGNORE_AFZ | pose_vel.FORCE | pose_vel.IGNORE_YAW | pose_vel.IGNORE_PX | pose_vel.IGNORE_PY | pose_vel.IGNORE_PZ;
	...
	point.position.x = P_r_I(0) ;
    	point.position.y = P_r_I(1) - 0.5;
    	point.position.z = P_r_I(2) + 0.5;
    	point.type_mask = point.IGNORE_VX | point.IGNORE_VY | point.IGNORE_VZ | point.IGNORE_AFZ | point.IGNORE_AFY | point.IGNORE_AFX;
    	point.coordinate_frame = 1;
    	point.yaw = 3.141592/2;

        local_pos_pub_mavros.publish(point);
